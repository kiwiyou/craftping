...
/// let response = ping(&mut stream, hostname, port, craftping::PROTOCOL_VERSION_NOT_SET).await.unwrap();
/// println!(
///     "{} of {} player(s) online",
...
{
    match ping_latest(stream, hostname, port, protocol_version).await {
        ok @ Ok(_) => ok,
